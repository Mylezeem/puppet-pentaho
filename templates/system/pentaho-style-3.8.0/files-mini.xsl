			...
			<xsl:if test="@visible='true'">
				<xsl:if test="@type='FILE.FOLDER'">
					...
					<tr style="cursor:pointer">
							<xsl:attribute name="onclick">var id='contentdiv<xsl:value-of select="position()"/>'; var e=document.getElementById(id); if(e.style.display=='none') { e.style.display='block'; document.getElementById('contentimg<xsl:value-of select="position()"/>').src='/pentaho-style<%= instance %>/images/btn_minus.png'; } else { e.style.display='none'; document.getElementById('contentimg<xsl:value-of select="position()"/>').src='/pentaho-style<%= instance %>/images/btn_plus.png'; } return false;</xsl:attribute>
		...
		<xsl:variable name="iconUrl">
			<xsl:choose>
				...
				<xsl:when test="class='org.pentaho.plugin.shark.SharkPublisher'">
					<xsl:text>/pentaho-style<%= instance %>/images/btn_shark.png</xsl:text>
				</xsl:when>
				<xsl:when test="class='org.pentaho.platform.engine.services.solution.SolutionPublisher'">
					<xsl:text>/pentaho-style<%= instance %>/images/btn_solutionrepos.png</xsl:text>
				</xsl:when>
				...
				<xsl:when test="class='org.pentaho.platform.engine.core.system.GlobalListsPublisher'">
					<xsl:text>/pentaho-style<%= instance %>/images/btn_globalactions.png</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>/pentaho-style<%= instance %>/images/btn_systemsettings.png</xsl:text>
				</xsl:otherwise>
