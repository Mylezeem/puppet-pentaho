			...
			if( key == 27 ) {
				...
				if( newImg ) {
					if( newImg.src.indexOf('folder_active.png') > -1 ) {
						newImg.src = '/pentaho-style<%= instance %>/images/folder_rollover.png';
					}
					else if( newImg.src.indexOf('file_active.png') > -1 ) {
						newImg.src = '/pentaho-style<%= instance %>/images/file_rollover.png';
					}
					if( oldImg ) {
						if( oldImg.src.indexOf('folder_rollover.png') > -1 ) {
							oldImg.src = '/pentaho-style<%= instance %>/images/folder_active.png';
						}
						else if( oldImg.src.indexOf('file_rollover.png') > -1 ) {
							oldImg.src = '/pentaho-style<%= instance %>/images/file_active.png';
						}
					}
					selectedRow = newRow;
				}
				...
				if( anchor ) {
					...
					if( target == '' || target == '.' ) {
						alert("this has been disabled to prevent possible XSS attack");
...
function flyTableSetup( title, desc, iconSrc, target, solution, path, filename, furl, properties, hasModifyAcl ) {
	...
	if( iconSrc && iconSrc != '' ) {
		...
		img.src='/pentaho-style<%= instance %>/images/spacer.gif';
	}
...
function changeFlyTab( tabNo ) {
	...
	if( tabNo == 1 ) {
		...
		document.getElementById('img-t1').src='/pentaho-style<%= instance %>/images/btn_info_active.png'; 
		document.getElementById('img-t2').src='/pentaho-style<%= instance %>/images/btn_actions.png'; 
	}
	if( tabNo == 2 ) {
		document.getElementById('flyTab1').style.display='none'; 
		document.getElementById('flyTab2').style.display='block'; 
		document.getElementById('img-t1').src='/pentaho-style<%= instance %>/images/btn_info.png'; 
		document.getElementById('img-t2').src='/pentaho-style<%= instance %>/images/btn_actions_active.png'; 
	}
}
