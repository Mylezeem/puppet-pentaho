			...
			<xsl:when test="/filters/subscriptions/@doSubscribe='true'">
				<xsl:choose>
					<xsl:when test="count(/filters/schedules/schedule) &lt; 0">
						<html>
							<head>
								<link rel='stylesheet' type='text/css' href='/pentaho-style<%= instance %>/pentaho.css'/>
								<title>
									<xsl:value-of select="title" disable-output-escaping="yes"/>
								</title>
								<link rel='stylesheet' type='text/css' href='/pentaho-style<%= instance %>/active/default.css'/>
							</head>
							<body>
