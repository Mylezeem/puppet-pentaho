...
<xsl:output method="html" encoding="UTF-8" />
			...
			<xsl:if test="@visible='true'">
				<xsl:if test="@type='FILE.FOLDER'">
					...
					<tr style="cursor:pointer">
							<xsl:attribute name="onclick">var id='contentdiv2<xsl:value-of select="position()"/>'; var e=document.getElementById(id); if(e.style.display=='none') { e.style.display='block'; document.getElementById('contentimg<xsl:value-of select="position()"/>').src='/pentaho-style<%= instance %>/images/btn_minus.png'; } else { e.style.display='none'; document.getElementById('contentimg<xsl:value-of select="position()"/>').src='/pentaho-style<%= instance %>/images/btn_plus.png'; } return false;</xsl:attribute>
						<td class="fileFolderDefault">
							...
							<img src="/pentaho-style<%= instance %>/images/btn_minus.png" border="0">
	...
	<xsl:variable name="title">
		...
		<xsl:variable name="iconUrl">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:text>/pentaho-style<%= instance %>/images/btn_systemsettings.png</xsl:text>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
 ...
 cursor: pointer;height:32px;padding:5px 5px 0px 43px;margin: 4px 0px 0px 0px; vertical-align: bottom;background-image: url(<xsl:value-of select="$iconUrl"/>);background-repeat: no-repeat;background-position: 0px 0px;</xsl:attribute>
	...
	<xsl:template name="doItem">
		...
		<xsl:variable name="iconUrl">
					<xsl:choose>
						<xsl:when test="@type='FILE.FOLDER'">/pentaho-style<%= instance %>/images/folder_active.png</xsl:when>
						<xsl:otherwise>/pentaho-style<%= instance %>/images/file_active.png</xsl:otherwise>
					</xsl:choose>
		</xsl:variable>
		<xsl:variable name="rolloverIconUrl">
					<xsl:choose>
						<xsl:when test="@type='FILE.FOLDER'">/pentaho-style<%= instance %>/images/folder_rollover.png</xsl:when>
						<xsl:otherwise>/pentaho-style<%= instance %>/images/file_rollover.png</xsl:otherwise>
					</xsl:choose>
		</xsl:variable>
